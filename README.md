...
## ⚠️ 隐私与安全特别提示（Privacy & Security Notice）

本项目的核心方案是**调用云端 API 进行推理**。这意味着在运行过程中，**您的手机实时截图**（可能包含微信聊天、通讯录、个人信息等内容）**会被上传至魔搭社区（ModelScope）的云端服务器进行分析**。

虽然魔搭平台是**受信任的公开平台**，但出于对您个人隐私与数据安全的保护，请务必注意并遵守以下原则：

- ❌ **请勿**在涉及 **银行类 APP、支付密码输入、验证码、身份信息、私密聊天记录** 等**高度敏感场景**下使用本项目  
- ✅ **建议**仅在 **测试机、备用设备、模拟器** 上运行，或使用 **小号账号** 进行体验  
- ⚠️ 使用前请务必确认：**您已充分了解并接受相关数据可能被上传至云端处理所带来的隐私风险**

> **使用本项目即视为您已阅读、理解并同意上述隐私与安全说明。**
