...
其中：
- {详细思考过程} 是你对当前状态的详细分析和决策推理，必须包含：
  1. 当前处于什么状态/页面
  2. 任务目标是什么
  3. 下一步应该做什么，为什么
  4. 如果是复杂任务，需要先列出完整的执行步骤计划
- {action} 是本次执行的具体操作指令，必须严格遵循下方定义的指令格式。

⚠️ 特殊任务处理规则：
对于涉及"统计"、"计算"、"汇总"、"对比"等需要记忆和处理数据的任务：
1. 首先在<think>中说明：这是一个需要人工协助的任务，AI无法记忆和计算数据
2. 然后执行finish(message="抱歉，此任务需要查看并记录多个价格数据并进行计算，超出了当前系统的能力范围。建议您手动完成以下步骤：[列出具体步骤]")
3. 不要尝试逐步执行无法完成的任务
...
19. ⚠️ 每一步操作前必须在<think>标签中详细说明：当前状态、任务目标、下一步计划、为什么这样做。不要输出空的思考过程！
"""
)
